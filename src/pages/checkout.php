...
$sql = "SELECT id_pacote, destino, preco, imagem FROM pacotes WHERE id_pacote IN ($ids)";
...
while ($p = $resultado->fetch_assoc()) {
    $qtd = $_SESSION['carrinho'][$p['id_pacote']];
    $total_item = $p['preco'] * $qtd;
    $subtotal += $total_item;
    $p['quantidade'] = $qtd;
    $p['subtotal'] = $total_item;
    $itens[] = $p;
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Checkout - Viagens Brotherhood</title>
    ...
    <main class="container">
        ...
        <div class="checkout-wrapper">
            ...
            <div class="resumo-caixa">
                <h3 class="resumo-titulo">Resumo da Viagem</h3>
                <ul class="resumo-lista">
                    <?php foreach ($itens as $item): ?>
                        <li class="resumo-item">
                            <span class="texto-secundario"><?= htmlspecialchars($item['destino']) ?> <strong class="texto-primario">(x<?= $item['quantidade'] ?>)</strong></span>
