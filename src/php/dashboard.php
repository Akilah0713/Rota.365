...
if ($_SERVER['REQUEST_METHOD' ] == 'POST') {
   ...
   $stmt = $conexão->prepare("UPDATE usuarios SET nome = ?, email =  ?, nascimento =  ? WHERE id_usuario =  ?");
...
if (!empty($_POST['senha_nova'])) {
   ...
   $stmt = $conexão->prepare("UPDATE usuarios SET senha_segura = ? WHERE id_usuario =  ?");
   $stmt->bind_param("si", $novaSenha_segura, $id);
   $stmt->execute();
   $stmt->close();
 }
 # Após salvar, redireciona para a mesma página para evitar reenvio de formulário
 header("Location: ../pages/conta.php?sucesso=1"); exit(); 
}
# Se o usuário apenas acessou a página (GET), Redireciona para o HTML do dashboard
header("Location: ../pages/conta.php"); exit(); 
?>