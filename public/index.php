<?php 
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <title>Rota.365 - Reservas de Viagem</title>
  <link rel="stylesheet" href="../src/assets/css/style.css">
</head>
<body>
  <header>
    <a id="logoClaro" href="index.php"><img src="../src/assets/imagens/logo-modo-claro-sem-fundo.png" alt="Logo clara"></a>
    <a id="logoEscuro" href="index.php" style="display: none;"><img src="../src/assets/imagens/logo-modo-escuro-sem-fundo.png" alt="Logo escura"></a>
    <nav class="menu">
       <div class="menu-container">
        
        <div class="menu-toggle" id="menuToggle">
          ☰
        </div>
        
        <ul id="menuLinks">
          <li><a href="index.php">🏠 Início</a></li>
          <li><a href="#nacionais">✈️ Nacionais</a></li>
          <li><a href="#internacionais">🌍 Internacionais</a></li>
          <li><a href="../src/pages/reservas.php">🧳 Reservas</a></li>
          <li><a href="#contato">📞 Contato</a></li>
        </ul>
      </div>
    </nav>
    <div class="menu dropdown">
      <button class="dropbtn"><img src="../src/assets/imagens/usuários modo claro.jpg"></button>
      <div class="dropdown-content">

        <?php if (isset($_SESSION['id_usuario'])): ?>
          <a href="../src/pages/conta.php" class="nav-link <?php echo ($pagina_atual === 'conta.php') ? 'ativo' : ''; ?>">Meu Perfil</a>
          <a href="../src/php/logout.php" class="nav-link">Sair</a>
        <?php else: ?>
          <a href="../src/pages/login.php" class="nav-link <?php echo ($pagina_atual === 'login.php') ? 'ativo' : ''; ?>">Entrar</a>
          <a href="../src/pages/criar-conta.php" class="nav-link <?php echo ($pagina_atual === 'cadastro.php') ? 'ativo' : ''; ?>">Cadastrar-se</a>
        <?php endif; ?>

        <!-- <div class="logado d-none">
          <a href="account.php">Minha conta</a>
          <a href="../src/pages/reservas.php">Reservas</a>
          <a href="sair">Sair </a>
        </div>
        <div class="deslogado">
          <a href="../src/pages/login.php">Entrar</a>
          <a href="../src/pages/ criar-conta.php">Cadastrar</a>
          <a href="./php/logout.php">Sair</a>
        </div> -->
    </div>
    </div>
    
    <button id="modoEscuro">
      <img src="../src/assets/imagens/">🌙</button>
  </header>

  <section class="carrossel">
    <div class="carrossel-container">
      <div class="slide ativo">
        <img src="../src/assets/imagens/pexels-architecture-1868667.jpg" alt="Destino 1">
      </div>
      <div class="slide">
        <img src="../src/assets/imagens/agustin-diaz-gargiulo-7F65HDP0-E0-unsplash.jpg" alt="Destino 2">
      </div>
      <div class="slide">
        <img src="../src/assets/imagens/john-wilander-keT-INMBlsY-unsplash.jpg" alt="Destino 3">
      </div>
      <div class="slide">
        <img src="../src/assets/imagens/leonard-cotte-R5scocnOOdM-unsplash.jpg" alt="Destino 4">
      </div>
      <button class="anterior">&#10094;</button>
      <button class="proximo">&#10095;</button>
    </div>
  </section>

  <section class="anuncio">
    <a class="anuncio-banner" href="https://www.latam.com/br/pt/" target="_blank">
      <img src="https://picsum.photos/1200/250?random=20" alt="Anúncio de Viagem">
      <div class="anuncio-texto">
        <h2>🌴 Ofertas Imperdiveís!</h2>
        <p>Descontos de até <strong>45%</strong> em pacotes nacionais. Aproveite antes que acabe!</p>
      </div>
    </a>
  </section>


  <main>
    <h2>Destinos Disponíveis</h2>
    <h2 id="nacionais">Viagens Nacionais</h2>

    <section class="destinos">
      <div class="card">
        <img src="../src/assets/imagens/cidade/rio de janeiro.jpg" alt="Rio de Janeiro">
        <h3>Rio de Janeiro</h3>
        <p>Brasília → Rio de Janeiro = 1.160 km</p>
        <p class="preco-novo">Executiva: R$ 928,00</p>
        <p class="preco-antigo">Padrão: R$ 580,00</p>
        <a href="../src/pages/reservas.php?destino=Rio%20de%20Janeiro">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Salvador 2.jpg" alt="Salvador">
        <h3>Salvador</h3>
        <p>Brasília → Salvador = 1.089 km</p>
        <p class="preco-novo">Executiva: R$ 579,00</p>
        <p class="preco-antigo">Padrão: R$ 390,00</p>
        <a href="../src/pages/reservas.php?destino=Salvador">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Porto Alegre.jpg" alt="PortoAlegre">
        <h3>Porto Alegre</h3>
        <p>Brasília → Porto Alegre = 1.620 km</p>
        <p class="preco-novo">Executiva: R$ 750,00</p>
        <p class="preco-antigo">Padrão: R$ 300,00</p>
        <a href="../src/pages/reservas.php?destino=Porto%20Alegre">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Esapirito santos.jpg" alt="Espirito Santo">
        <h3>Espirito Santo</h3>
        <p>Brasília → Espirito Santo = 940 km</p>
        <p class="preco-novo">Executiva: R$ 600,00</p>
        <p class="preco-antigo">Padrão: R$ 250,00</p>
        <a href="../src/pages/reservas.php?destino=Espirito%20Santo">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/icaraí.jpg" alt="Icaraí de Amontada">
        <h3>Icaraí de Amontada</h3>
        <p>Brasília → Icaraí de Amontada = 1.440 km</p>
        <p class="preco-novo">Executiva: R$ 1.152,00</p>
        <p class="preco-antigo">Padrão: R$ 720,00</p>
        <a href="../src/pages/reservas.php?destino=Icara%C3%AD%20de%20Amontada">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Nobres.jpg" alt="Nobres">
        <h3>Nobres</h3>
        <p>Brasília → Nobres = 1.600 km</p>
        <p class="preco-novo">Executiva: R$ 1.280,00</p>
        <p class="preco-antigo">Padrão: R$ 800,00</p>
        <a href="../src/pages/reservas.php?destino=Nobres">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/rioGrandeDoSul.jpg" alt="Rio Grande do Sul">
        <h3>Rio Grande do Sul</h3>
        <p>Brasília → Rio Grande do Sul = 2.022 km</p>
        <p class="preco-novo">Executiva: R$ 1.617,00</p>
        <p class="preco-antigo">Padrão: R$ 1.011,00</p>
        <a href="../src/pages/reservas.php?destino=Rio%20Grande%20do%20Sul">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/balneario.jpg" alt="balneario">
        <h3>Balneário Camboriú</h3>
        <p>Brasília → Balneário Camboriú = 1.596.6 km</p>
        <p class="preco-novo">Executiva: R$ 1.276,00</p>
        <p class="preco-antigo">Padrão: R$ 798,00</p>
        <a href="../src/pages/reservas.php?destino=Balne%C3%A1rio%20Cambori%C3%BA">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/jericoacoara.jpg" alt="Jericoacoara">
        <h3>Jericoacoara</h3>
        <p>Brasília → Jericoacoara = 2.096.4 km</p>
        <p class="preco-novo">Executiva: R$ 1.676,00</p>
        <p class="preco-antigo">Padrão: R$ 1.048,00</p>
        <a href="../src/pages/reservas.php?destino=Jericoacoara">Reservar</a>
      </div>
    </section>
    <section class="anuncio pequeno">
      <a class="anuncio-banner" href="https://www.trivago.com.br" target="_blank">
        <img src="https://picsum.photos/1000/200?random=50" alt="Anúncio Intermediário">
        <div class="anuncio-texto">
        <h3>🏨 Parceria com Trivago</h3>
        <p>Reserve seu hotel com até 30% de desconto em viagens Rota.365.</p>
        </div>
      </a>
    </section>

    <h2 id="internacionais">Viagens Internacionais</h2>
    <section class="destinos">
      <div class="card">
        <img src="../src/assets/imagens/cidade/Veneza.jpg" alt="Veneza">
        <h3>Veneza</h3>
        <p>Brasília → Veneza = 7.454 km</p>
        <p class="preco-novo">Executiva: R$ 5.963,00</p>
        <p class="preco-antigo">Padrão: R$ 3.727,00</p>
        <a href="../src/pages/reservas.php?destino=Veneza">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Seoul 2.jpg" alt="Seoul">
        <h3>Seoul</h3>
        <p>Brasília → Seoul = 17.620 km</p>
        <p class="preco-novo">Executiva: R$ 9.000,00</p>
        <p class="preco-antigo">Padrão: R$ 5.000,00</p>
        <a href="../src/pages/reservas.php?destino=Seoul">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Istambul.jpg" alt="Istambul">
        <h3>Istambul</h3>
        <p>Brasília → Istambul = 10.095 km</p>
        <p class="preco-novo">Executiva: R$ 6.500,00</p>
        <p class="preco-antigo">Padrão: R$ 3.000,00</p>
        <a href="../src/pages/reservas.php?destino=Istambul">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Paris 2.jpg" alt="Paris">
        <h3>Paris</h3>
        <p>Brasília → Paris = 8.700 km</p>
        <p class="preco-novo">Executiva: R$ 7.000,00</p>
        <p class="preco-antigo">Padrão: R$ 4.800,00</p>
        <a href="../src/pages/reservas.php?destino=Paris">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Orlando 2.jpg" alt="Orlando">
        <h3>Orlando</h3>
        <p>Brasília → Orlando = 8.700 km</p>
        <p class="preco-novo">Executiva: R$ 4.500,00</p>
        <p class="preco-antigo">Padrão: R$ 2.700,00</p>
        <a href="../src/pages/reservas.php?destino=Orlando">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/singapura.jpg" alt="singapura">
        <h3>Singapura</h3>
        <p>Brasília → Singapura = 8.700 km</p>
        <p class="preco-novo">Executiva: R$ 8.800,00</p>
        <p class="preco-antigo">Padrão: R$ 7.600,00</p>
        <a href="../src/pages/reservas.php?destino=Singapura">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Dubai 2.jpg" alt="Dubai">
        <h3>Dubai</h3>
        <p>Brasília → Dubai = 12.059 km</p>
        <p class="preco-novo">Executiva: R$ 7.200,00</p>
        <p class="preco-antigo">Padrão: R$ 3.500,00</p>
        <a href="../src/pages/reservas.php?destino=Dubai">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/Tokyo.jpg" alt="Tokyo">
        <h3>Tokyo</h3>
        <p>Brasília → Tokyo = 16.808 km</p>
        <p class="preco-novo">Executiva: R$ 13.446,00</p>
        <p class="preco-antigo">Padrão: R$ 8.404,00</p>
        <a href="../src/pages/reservas.php?destino=Tokyo">Reservar</a>
      </div>

      <div class="card">
        <img src="../src/assets/imagens/cidade/moscou.jpg" alt="Moscou">
        <h3>Moscou</h3>
        <p>Brasília → Moscou = 11.179 km</p>
        <p class="preco-novo">Executiva: R$ 8.943,00</p>
        <p class="preco-antigo">Padrão: R$ 5.588,00</p>
        <a href="../src/pages/reservas.php?destino=Moscou">Reservar</a>
      </div>
    </section>
  </main>

  <section class="anuncio rodape">
  <a class="anuncio-banner" href="https://www.booking.com" target="_blank">
    <img src="https://picsum.photos/1200/300?random=90" alt="Anúncio de Rodapé">
    <div class="anuncio-texto">
      <h3>🌎 Viaje com conforto e economia!</h3>
      <p>Conheça nossos parceiros e garanta descontos exclusivos.</p>
    </div>
  </a>
</section>


  <footer id="contato">
    <p>Contato: contato@rota365.com | Tel: (61) 4002-8922</p>
    <p>Redes sociais: @rota365</p>
    <p>Todos os Direitos reservados</p>
  </footer>

  <script src="../src/assets/js/script.js"></script>
</body>
</html>